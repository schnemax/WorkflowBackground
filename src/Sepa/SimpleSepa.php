...
final class SimpleSepa implements SepaService
{
    ...
    public function generate_sepa_pain001(array $p, int $docId, $repo, string $painVersion = 'pain.001.001.09'): string
    {
        // ---- helpers ----
        $fmtAmt = static fn($x): string => number_format((float)$x, 2, '.', '');
        $sanitizeIban = static fn(string $x): string => strtoupper(preg_replace('/\s+/', '', $x ?? ''));
        $clip = static fn(string $x, int $n): string => mb_substr($x ?? '', 0, $n);

        $tz = new \DateTimeZone(getenv('TZ') ?: 'Europe/Berlin');
        ...
        $debtorIban = $sanitizeIban((string)$this->repo->DLookup("iban", "konto", $query));
        $debtorBic  = (string)$this->repo->DLookup("bic",  "konto", $query); // optional

        // ---- payment data ----
        $amount = (float)($p['invoice_amount'] ?? 0.0);
        $creditorName = self::sanitize((string)($p['issuer_name'] ?? ''), 70);
        $creditorIban = $sanitizeIban((string)($p['issuer_iban'] ?? ''));
        $creditorBic  = (string)($p['issuer_bic'] ?? ''); // optional

        // Purpose / remittance (max 140)
        $purpose = trim((string)($p['payment_purpose'] ?? ''));
        if ($purpose === '') {
            $purpose = 'Rechnung ' . ($p['invoice_number'] ?? '') ?: ('Unser Dokument ' . $docId);
        }
        $purpose = self::sanitize($purpose . ' DMSID:' . $docId, 140);

        // End-to-End-ID (≤ 35)
        $endToEnd = $clip((string)($p['end_to_end'] ?? 'NOTPROVIDED'), 35);
        ...
        // ---- pain namespace ----
        // Erlaubt: 'pain.001.001.03' oder 'pain.001.001.09'
        //$pain = $painVersion;
        $pain = "pain.001.001.03";
        $ns = "urn:iso:std:iso:20022:tech:xsd:{$pain}";
        ...
        // schemaLocation (einige Banken verlangen das; schadet nicht)
        $doc->setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:schemaLocation', $ns . ' ' . $pain . '.xsd');
        $xml->appendChild($doc);
        ...
        $grp->appendChild($xml->createElementNS($ns, 'CreDtTm', $nowUtc));
        ...
        $grp->appendChild($ip);
        $ip->appendChild($xml->createElementNS($ns, 'Nm', $clip($debtorName, 70)));
        // (Optional) Falls deine Bank ein organisatorisches Kennzeichen will:
        // $id = $xml->createElementNS($ns, 'Id'); $ip->appendChild($id);
        // $orgId = $xml->createElementNS($ns, 'OrgId'); $id->appendChild($orgId);
        // $othr = $xml->createElementNS($ns, 'Othr'); $orgId->appendChild($othr);
        // $othr->appendChild($xml->createElementNS($ns, 'Id', 'DEINE-KUNDEN-ID'));
        ...
        // (Optional – nur falls ihr Instant wollt und Bank es unterstützt)
        // $lcl = $xml->createElementNS($ns, 'LclInstrm'); $tti->appendChild($lcl);
        // $lcl->appendChild($xml->createElementNS($ns, 'Prtry', 'INST'));
        ...
        $pi->appendChild($dbtr);
        $dbtr->appendChild($xml->createElementNS($ns, 'Nm', $clip($debtorName, 70)));
        ...
        // --- Debtor Agent (immer vorhanden; BIC oder Othr/Id NOTPROVIDED) ---
        $dbtrAgt = $xml->createElementNS($ns, 'DbtrAgt');
        $pi->appendChild($dbtrAgt);
        $fi = $xml->createElementNS($ns, 'FinInstnId');
        $dbtrAgt->appendChild($fi);

        // Variante ohne BIC: Othr/Id = NOTPROVIDED
        $othr = $xml->createElementNS($ns, 'Othr');
        $fi->appendChild($othr);
        $othr->appendChild($xml->createElementNS($ns, 'Id', 'NOTPROVIDED'));
        ...
        // Creditor Agent: analog zur Debtor-Seite
        if (1 === 2) {
            if ($pain === 'pain.001.001.03' && $creditorBic !== '') {
        ...
        // (Optional: wenn deine Bank trotz .09 einen Agent will, dann oben die Bedingung anpassen)

        $cdtr = $xml->createElementNS($ns, 'Cdtr');
        $tx->appendChild($cdtr);
        $cdtr->appendChild($xml->createElementNS($ns, 'Nm', $clip($creditorName, 70)));
        ...
        if ($purpose !== '') {
            ...
            $tx->appendChild($rmt);
            $rmt->appendChild($xml->createElementNS($ns, 'Ustrd', $purpose)); // ≤ 140
            // (Optional für strukturierte Verwendungszwecke in .09)
            // $strd = $xml->createElementNS($ns, 'Strd'); $rmt->appendChild($strd);
            // $cdtrRefInf = $xml->createElementNS($ns, 'CdtrRefInf'); $strd->appendChild($cdtrRefInf);
            // $tp = $xml->createElementNS($ns, 'Tp'); $cdtrRefInf->appendChild($tp);
            // $cd = $xml->createElementNS($ns, 'CdOrPrtry'); $tp->appendChild($cd);
            // $cd->appendChild($xml->createElementNS($ns, 'Cd', 'SCOR'));
            // $cdtrRefInf->appendChild($xml->createElementNS($ns, 'Ref', 'DEIN-REFERENZCODE'));
        }
        ...
        \App\Log::j('INFO', 'SEPA created', ['doc' => $docId, 'file' => $file, 'pain' => $pain]);
        return $file;
    }


    public function generate_sepa_pain001_old_03(array $p, int $docId, $repo): string
    {
